...
async function deleteCategory(id){
    try {
        const token = getCookie('accesstoken');
        const response = await fetch(`${baseURL}inventory/category/${id}/`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
...
// Fetch items, optionally filtering by category
async function getItems(categoryId){
    const token = getCookie('accesstoken')
    const url = categoryId
        ? `${baseURL}inventory/item/?category=${categoryId}`
...
async function deleteItem(id) {
    const token = getCookie('accesstoken');
    const response = await fetch(`${baseURL}inventory/item/${id}/`, {
        method: 'DELETE',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    if (response.ok) {
        ...
        console.error(`Failed to delete item ${id}:`, errorText);
