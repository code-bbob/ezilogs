...
function Profit() {
  ...
  // Add technicians list state for dropdown
  ...
  const someProfitFunction = async () => {
    try {
      ...
      if (searchParams.has('start_date') && searchParams.has('end_date')) {
        ...
        let query = `start_date=${formatDate}&end_date=${formatDate}`;
        // Include technician filter if selected
        if (searchParams.has('technician')) {
          query += `&technician=${searchParams.get('technician')}`
        }
        const token = getCookie('accesstoken')
        const res = await fetch(
          `${baseURL}enterprise/profit?${query}`,
  ...
  // Fetch technicians for selection dropdown
  useEffect(() => {
    const fetchTechnicians = async () => {
      try {
        const token = getCookie('accesstoken')
        const res = await fetch(`${baseURL}enterprise/techs/`, {
          ...
          <div className='flex gap-2'>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) params.set('technician', e.target.value)
                else params.delete('technician')
                router.push(`/repair/profit?${params.toString()}`)
              }}
